...
struct MessageBubble: View {
        ...
        HStack {
            ...
            VStack(alignment: message.role == "user" ? .trailing : .leading, spacing: 8) {
                if let imageUrl = message.imageUrl {
                    if imageUrl.hasPrefix("data:"), let data = dataFrom(base64: imageUrl), let uiImage = UIImage(data: data) {
    ...
    private func dataFrom(base64: String) -> Data? {
        let parts = base64.components(separatedBy: ",")
        guard parts.count > 1, let dataString = parts.last else { return nil }
