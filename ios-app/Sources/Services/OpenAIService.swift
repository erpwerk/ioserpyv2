    ...
    private func generateImage(prompt: String, apiKey: String) {
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            guard let imageDownloadURL = URL(string: imageUrl) else {
                DispatchQueue.main.async {
                    self?.onMessage?("⚠️ Bild-URL ungültig.")
                    self?.onComplete?()
                }
                return
            }
            ...
            URLSession.shared.dataTask(with: imageDownloadURL) { [weak self] imageData, imageResponse, imageError in
                guard let imageData = imageData, imageError == nil else {
                    DispatchQueue.main.async {
                        self?.onMessage?("⚠️ Bild konnte nicht heruntergeladen werden: \(imageError?.localizedDescription ?? "Unbekannter Fehler").")
                        self?.onComplete?()
                    }
                    return
                }
                ...
                let mimeType = imageResponse?.mimeType ?? "image/png" // Default to png if mimeType is unknown
