...
class OpenAIService: NSObject, LLMProvider, URLSessionDataDelegate {
    ...
    private func generateImage(prompt: String, apiKey: String) {
        ...
        let body: [String: Any] = [
            ...
            "n": 1,
            "size": "1024x1024",
            "response_format": "b64_json" // Prefer base64 for 2026 consistency
        ]
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            guard let data = data,
                  ...
                  let firstObject = dataArray.first else {
                
                let rawBody = data.flatMap { String(data: $0, encoding: .utf8) } ?? "No data"
                DispatchQueue.main.async {
                    self?.onMessage?("⚠️ Bildgen Parser Error. Rohdaten: \(rawBody)")
                    self?.onComplete?()
                }
                return
            }
            
            // Handle b64_json (Preferred)
            if let base64 = firstObject["b64_json"] as? String {
                DispatchQueue.main.async {
                    self?.onMessage?("[IMAGE]data:image/png;base64,\(base64)")
                    self?.onComplete?()
                }
                return
            }
            ...
            // Fallback to URL
            if let imageUrl = firstObject["url"] as? String, let url = URL(string: imageUrl) {
                URLSession.shared.dataTask(with: url) { [weak self] imageData, imageResponse, imageError in
                    if let imageData = imageData, imageError == nil {
                        let b64 = imageData.base64EncodedString()
                        let mime = imageResponse?.mimeType ?? "image/png"
                        DispatchQueue.main.async {
                            self?.onMessage?("[IMAGE]data:\(mime);base64,\(b64)")
                            self?.onComplete?()
                        }
                    } else {
                        DispatchQueue.main.async {
            ...
            DispatchQueue.main.async {
                self?.onMessage?("⚠️ Kein Bildformat (URL/Base64) gefunden.")
                self?.onComplete?()
            }
        }.resume()
    }
