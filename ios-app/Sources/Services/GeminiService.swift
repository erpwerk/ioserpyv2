...
class GeminiService: NSObject, LLMProvider, URLSessionDataDelegate {
    ...
    func generateStream(prompt: String, model: String, apiKey: String, onMessage: @escaping (String) -> Void, onComplete: @escaping () -> Void) {
        ...
        // Stable Branching (Abzweigung) to Imagen 3
        let lowerPrompt = prompt.lowercased()
        if lowerPrompt.contains("bild") || lowerPrompt.contains("generier") || lowerPrompt.contains("erstell") || lowerPrompt.contains("photo") {
            generateImagen3(prompt: prompt, apiKey: apiKey)
            return
        }
        
        // Default to stable 3.0 Flash as requested
        let correctedModel = model.contains("gemini") ? model : "gemini-3.0-flash"
    ...
    private func generateImagen3(prompt: some StringProtocol, apiKey: String) {
        // Imagen 3 is the stable 2026 high-end image model
        let urlString = "https://generativelanguage.googleapis.com/v1beta/models/imagen-3:generateContent?key=\(apiKey)"
        guard let url = URL(string: urlString) else { return }
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            if let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode != 200 {
                ...
                DispatchQueue.main.async {
                    self?.onMessage?("⚠️ Gemini Image Error (\(httpResponse.statusCode)): \(errorBody)")
                    self?.onComplete?()
                }
                return
            }
            ...
            guard let data = data,
                ...
                DispatchQueue.main.async {
                    self?.onMessage?("⚠️ Imagen 3 Parser Error. Check logs.")
                    self?.onComplete?()
                }
                return
            }
