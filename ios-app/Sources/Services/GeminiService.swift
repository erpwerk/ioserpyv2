...
class GeminiService: NSObject, LLMProvider, URLSessionDataDelegate {
    ...
    func urlSession(_ session: URLSession, dataTask: URLSessionDataTask, didReceive data: Data) {
        ...
        // Split by the start of a JSON object '{' but keep the '{'
        // This is a common way to handle streamed JSON fragments
        ...
        for fragment in fragments {
            ...
            let jsonString = "{" + fragment
            // Try to find the matching closing brace or just try parsing progressively stripped versions
            // But usually, pieces are small enough or delimited by commas in the array
            
            var cleaned = jsonString.trimmingCharacters(in: .whitespacesAndNewlines)
            if cleaned.hasSuffix(",") { cleaned.removeLast() }
            if cleaned.hasSuffix("]") { cleaned.removeLast() }
            
            guard let chunkData = cleaned.data(using: .utf8) else { continue }
