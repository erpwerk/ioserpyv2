...
class GeminiService: NSObject, LLMProvider, URLSessionDataDelegate {
    ...
    func generateStream(prompt: String, model: String, apiKey: String, onMessage: @escaping (String) -> Void, onComplete: @escaping () -> Void) {
        ...
        if lowerPrompt.contains("bild") || lowerPrompt.contains("generier") || lowerPrompt.contains("erstell") || lowerPrompt.contains("photo") {
            generateFlashImage(prompt: prompt, apiKey: apiKey)
            return
        }
    ...
    private func generateFlashImage(prompt: some StringProtocol, apiKey: String) {
        // Updated to preview version as suggested by the user's API error feedback
        let urlString = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-image:generateContent?key=\(apiKey)"
        guard let url = URL(string: urlString) else { return }
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            guard let data = data,
                ...
                DispatchQueue.main.async {
                    self?.onMessage?("⚠️ Flash Image Parser Error. Check logs.")
                    self?.onComplete?()
                }
                return
            }
