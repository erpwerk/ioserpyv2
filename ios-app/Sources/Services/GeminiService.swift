...
class GeminiService: NSObject, LLMProvider, URLSessionDataDelegate {
    ...
    func generateStream(prompt: String, model: String, apiKey: String, onMessage: @escaping (String) -> Void, onComplete: @escaping () -> Void) {
        ...
        if lowerPrompt.contains("bild") || lowerPrompt.contains("generier") || lowerPrompt.contains("erstell") || lowerPrompt.contains("photo") {
            generateImage25(prompt: prompt, apiKey: apiKey)
            return
        }
        
        // Use stable 2.5 Flash as requested
    ...
    private func generateImage25(prompt: some StringProtocol, apiKey: String) {
        // gemini-2.5-flash-image is the standard 2026 free-tier image model (Nano Banana)
        // NOTE: Even in the Free Tier, Google usually requires a linked billing account for image generation.
        let urlString = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-image:generateContent?key=\(apiKey)"
        guard let url = URL(string: urlString) else { return }
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            if let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode != 200 {
                ...
                // Specific 2026 error handling for Free Tier image generation
                ...
                if errorBody.contains("billing") || errorBody.contains("free tier") || httpResponse.statusCode == 403 {
                    userFriendlyError = "⚠️ Gemini Bildgen-Fehler: Für die Bild-API ist 2026 meist ein verknüpftes Google Cloud Billing-Konto erforderlich (auch im Free Tier!)."
                } else if httpResponse.statusCode == 429 {
                    userFriendlyError = "⚠️ Gemini Bildgen-Limit erreicht. Bitte warte bis morgen oder nutze GPT-4.1-mini."
                } else {
            ...
            guard let data = data,
                ...
                DispatchQueue.main.async {
                    self?.onMessage?("⚠️ Bildgen Parser Error. Evtl. Limit erreicht oder ungültiges Format.")
                    self?.onComplete?()
                }
                return
            }
