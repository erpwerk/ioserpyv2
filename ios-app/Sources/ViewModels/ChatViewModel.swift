...
class ChatViewModel: ObservableObject {
    ...
    let models = ["gpt-4.1-mini", "gemini-2.5-flash"]
    ...
    private func startStreaming(prompt: String) {
        ...
        let isImageRequest = lowerPrompt.contains("bild") || lowerPrompt.contains("generier") || lowerPrompt.contains("erstell") || lowerPrompt.contains("photo")
        ...
        if selectedModel.contains("gpt") {
            ...
            // ROUTING: Switch to OpenAI for Gemini image requests
            provider = openAIService
            apiKey = openAIKey
            model = "gpt-4.1-mini"
            appendAssistantContent(id: assistantMessage.id, chunk: "*(Bilderstellung via OpenAI...)*\n")
        } else {
